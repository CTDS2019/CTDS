...
extends Task[CallSequence](global) {
	...
	def computeSequenceCandidate = {
		...
		//println("观察原参数列表"+param)
		//println("观察原序列"+newSequence)
		param = findVarOfType(typ, newSequence, nullAllowed)
		//print("参数" + typ)
		if (param.isDefined) {
				//println("观察新添加的参数"+param)
				//println("观察新添加参数的序列"+newSequence)
		    Some(newSequence)
		} else {
				//println("添加参数失败"+param)
		    None
		}
	}
	...
	private def findVarOfType(typ: String, sequence: CallSequence, nullAllowed: Boolean): Option[Variable] = {
		...
		if (sequence.types.contains(typ) && global.random.nextBool) { // reuse existing var of this type
			...
			} else if (global.typeProvider.primitiveProvider.isArray(typ)){
				//print("参数" + typ)
				//println("参数" + global.typeProvider.primitiveProvider.nextArray(typ))
				return Some(new ArrayConstant(typ.charAt(1), global.typeProvider.primitiveProvider.nextArray(typ)))
