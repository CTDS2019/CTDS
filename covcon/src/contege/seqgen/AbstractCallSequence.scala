...
abstract class AbstractCallSequence[MyType](global: GlobalState) {
	...
	def execute(var2Object: Map[Variable, Object]): Option[Throwable] = {
		def obj(v: Variable): Object = v match {
			case c: Constant => c.value //基本类型或字符串
			case NullConstant => null
			case c: ArrayConstant => c.arr
			case _ => var2Object(v) // 取出一个对象
		}
	...
	def execute2(var2Object: Map[Variable, Object]): Option[Throwable] = {
		def obj(v: Variable): Object = v match {
			case c: Constant => c.value //基本类型或字符串
			case c: ArrayConstant => c.arr
			case NullConstant => null
		...
		calls.foreach(call => {
			...
			val argObjs = call.args.map(argVar => obj(argVar))
			val result = call.atom.execute2(receiverObj, argObjs, call.downcastCls)
			result match {
				case Normal(retVal) => call.retVal match {
					case Some(retValVar) => var2Object.put(retValVar, retVal)
					case None => //assert(retVal == null, "Call should return void when call.retVal is undefined")
	...
	def execute2: Option[Throwable] = execute2(Map[Variable, Object]())
		...
		if (v == NullConstant) "null"
			...
			v.asInstanceOf[Constant].value match {
			    case s: String =>
						...
						"\""+s+"\""
			    case nonString =>
						//print("观察直接量参数"+nonString.toString)
						nonString.toString

			}
		}else if (v.isInstanceOf[ArrayConstant]){
			//if (v.asInstanceOf[ArrayConstant].ty.equals(''))
			v.asInstanceOf[ArrayConstant].ty match{
				case 'B' => "new Array[Byte](3)"
				case 'C' => "new Array[Char](3)"
				case 'D' => "new Array[Double](3)"
				case 'F' => "new Array[Float](3)"
				case 'I' => "new Array[Int](3)"
				case 'J' => "new Array[Long](3)"
				case 'S' => "new Array[Short](3)"
				case 'Z' => "new Array[Boolean](3)"
			}
...
class Constant(val value: Object) extends Variable // primitive type or String 基本类型或字符串
class ArrayConstant(val ty : Char, val arr: Object) extends Variable // primitive type or String 基本类型或字符串的数组
object NullConstant extends Variable // primitive type or String

// atom：要执行的操作(例如，方法调用或字段访问)。
// receiver：一种类型
// args：参数集合
// retVal：返回值
// downcastType：向下转换类型？子类类型？
