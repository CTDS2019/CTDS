...
abstract class Atom(val receiverType: Class[_]) {
	...
	def execute2(receiver: Object, args: Seq[Object], downcastCls: Option[Class[_]]): ExecutionResult
	def nbParams: Int
...
class MethodAtom(receiverType: Class[_], val method: Method) extends Atom(receiverType) {
	...
	override def execute2(receiver: Object, args: Seq[Object], downcastCls: Option[Class[_]]): ExecutionResult = {
		val result = TimeoutRunner.runWithTimeout2(() => {
			try {
				if (receiver == null && !isStatic) Exception(NullAsReceiverException)
				else {
					val r = method.invoke(receiver, args:_*)
